...
folders_list = []
listAllFolderName(folders_list, PATH_SRC)

ignored_list = ["setup", "view", "debug"]

folder_list_to_process = []
for element in os.listdir(PATH_SRC):
    if not "." in element and element not in ignored_list:
        folder_list_to_process.append(PATH_SRC + element)

# for folder in folders_list:
#     # print(list(folders_in(folder)))
#     folder_list = list(folders_in(folder))
#     if not folder_list and "/home/pi/ATE_release/newATE/setup/" not in folder:
#         folder_list_to_process.append(folder)

print("folder_list_to_process: ", folder_list_to_process)
count = 0

try:
    for folder in folder_list_to_process:
        if folder:
            count += 1
            locals()["Thread_" + str(count)] = threading.Thread(target=convertEntireFolder, args=[folder])
            locals()["Thread_" + str(count)].start()
